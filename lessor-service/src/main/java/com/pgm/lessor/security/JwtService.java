...
/**
 * Issues and validates access tokens. Both lessor-service and renter-service sign and
 * verify with the same HS256 secret (app.jwt.secret) so either service can validate a
 * token issued by the other - useful if a future gateway needs to check auth without
 * calling back into whichever service issued the token.
 */
@Service
