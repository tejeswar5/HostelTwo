...
import com.pgm.lessor.entity.AuditLog;
import com.pgm.lessor.repository.AuditLogRepository;
import com.pgm.lessor.security.JwtAuthenticationToken;
...
public class AuditAspect {
     ...
     * {@code Long}, e.g. from a decision method keyed by id) or a response DTO whose
     * id we read reflectively. DTOs are Java records, which expose an id-shaped
     * accessor named after the component itself ({@code id()}), not the JavaBean
     * {@code getId()} - so both are tried. Deliberately does not fall back to
     * scanning method arguments for a {@code Long}: a mutation like
     * {@code raise(Long renterId, RaiseComplaintRequest request)} has a Long argument
     * that is not the entity being audited, and guessing from it silently logs the
     * wrong id.
     */
